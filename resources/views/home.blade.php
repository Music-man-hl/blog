        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                ...
                @foreach($articles as $article)
                    <div class="z-article-vertical card">
                        <img src="{{ $article->cover ?: 'default.jpg'}}" class="img-fluid z-cover" alt="max">
                        <div class="z-content">
                            @foreach($article->tags as $tag)
                                <span class="label label-info" style="font-size:11px;padding:1px 5px">{{ $tag->name }}</span>
                            @endforeach
                            <p class="z-title">{{ $article->title }}</p>
                            <p class="z-info">- 发表于 {{ $article->created_at->toDateString() }} · 最后访问 {{ $article->updated_at->diffForHumans() }} -</p>
                            <p class="z-intro">{!! str_limit(strip_tags($article->content), 360) !!}</p>
                            <div class="z-center-horizontal">
